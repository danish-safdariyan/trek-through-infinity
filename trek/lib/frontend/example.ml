...
let shadow_box =
  let open Style in
  create ~background:(opaque_bg Draw.grey) ~shadow:(mk_shadow ()) ()
...
let example1v () =
  ...
  let h = 50 in
  let title =
    W.rich_text ~size:20 ~h:30 Text_display.(page [ para "Text samples" ])
  in
  let td =
    W.rich_text ~h Text_display.(page [ underline (para "Original:"); example ])
  in
  let td_normal =
    W.rich_text ~h
      (let open Text_display in
       [ underline (para "Force normal style:"); normal example ])
  in
  let td_bold =
    W.rich_text ~h
      Text_display.(page [ underline (raw "Force bold:"); bold example ])
  in
  let td_italic =
    W.rich_text ~h
      Text_display.(page [ underline (para "Force italic:"); italic example ])
  in
  let box = W.box () in
  let layout =
    L.tower_of_w [ b; title; td; td_normal; td_bold; td_italic; box ]
  in
...
let example3 () =
  ...
  let action w1 w2 _ =
    ...
    set_check_state w2 (get_state w1)
  in
...
let desc5 =
  "We pack two layouts in a (horizontal) flat. Layouts have names (if \
   DEBUG=true, you may press CTRL-I to see the info for each layout). The \
   second button is draggable."
...
let example6 () =
  let b = W.check_box () in
  let l =
    W.label ~fg:Draw.(opaque (find_color "firebrick")) "Merry Christmas !"
  in
...
let example7 () =
  (* TODO à vérifier ! parfois ce n'est pas synchro *)
  let b = W.check_box () in
  let l = W.label "Merry Christmas !" in
  let action w1 w2 _ =
    print_endline "action";
    let text =
      if W.get_state w1 then "Happy New Year !" else "Merry Christmas !"
    in
    Label.set (W.get_label w2) text
  in
...
let example9 () =
  ...
  (* this prevents the box around the button to expand when resizing the
     layout/window.*)
...
let example10 () =
  ...
  let b2 = W.check_box () in
  let l1 =
    L.flat_of_w ~name:"Window#1" [ b1; W.label "Show the second window" ]
  in
  ...
  L.set_window_pos l1 (200, 200);
  L.set_window_pos l2 (400, 400);
  run board

let desc11 =
  "Two connected windows: the button in the first window sets the check_box in \
   the second window."

let example11 () =
  (* attention ne marche pas avec DEBUG=false !! OK problème résolu: le main
     thread ne laissait pas assez de temps aux autres *)
  let b1 = W.check_box () in
  let b2 = W.check_box () in
  let action w1 w2 _ = W.set_check_state w2 (W.get_state w1) in
  let c = W.connect b1 b2 action T.buttons_down in
  (* W.add_connection b1 c;  *)
  (* TODO à faire autom *)
  ...
  let w2 =
    Window.create
      ~on_close:(fun _ ->
        print_endline
          "I won't close if my sister is still alive! Press ESC to quit.")
      l2
  in
  let shortcuts = shortcuts_of_list [ exit_on_escape ] in
...
let example13 () =
  ...
  let knob =
    L.superpose ~center:true [ L.resident ~background s; L.resident text ]
  in
...
let example14 () =
  ...
  let action w1 w2 _ =
    ...
    Label.set (W.get_label w2) (sprintf "You have selected: %u%% " x)
  in
  let events =
    List.flatten
...
let desc15 =
  "Two (independent) clocks in text label; one is starting automatically."
...
let example15 () =
  let clock w1 _ ev =
    let rec loop () =
      ...
      (* printf "Drift = %f\n" drift; *)
      (* we try to keep the drift near 0.5 *)
      if drift < 0.45 then Thread.delay (0.51 -. drift)
      else if drift > 0.55 then Thread.delay (1.49 -. drift)
        (* we are too late *)
...
let example16 () =
  ...
  let d =
    W.button ~bg_off (* ~bg_on *)
      ~bg_over ~kind:Button.Switch ~label_on:(Label.icon ~fg "train")
...
let example17 () =
  let img = W.image ~noscale:true ~bg:Draw.(opaque white) image_file in
...
let example19 () =
  ...
  let tabs =
    Tabs.create ~slide:Avar.Right ~adjust:Layout.Nothing
      [ ("Check box", tab3); ("Image", tab1); ("Text entry", tab2) ]
  in
...
let example21 () =
  ...
  let close_btn =
    W.button ~border_radius:3 ~border_color:Draw.(opaque blue) "Close"
  in
  let popup = L.tower_of_w [ l; ti; close_btn ] in
  let layout = L.tower_of_w [ b; td ] in
  let screen =
    Popup.attach ~show:false ~bg:Draw.(set_alpha 220 (pale green)) layout popup
  in
  (* TODO use actions *)
  let button =
    W.button ~kind:Button.Switch ~border_radius:4
      ~border_color:Draw.(opaque grey)
      "Popup"
  in
  let release b =
    let state = Button.state (W.get_button b) in
    L.set_show popup state;
    L.set_show screen state
  in
  ...
  (* let c = W.connect_main close_btn button (fun _ b _ -> close b) T.buttons_up
     in *)
...
(* BUG: when you hide the inner box, and then the outer box; and then open the
   outer box again, then the offset position of the innex box isn't correct.
   After two clicks, it comes back to the right position. *)
...
let example22 () =
  ...
  let action room w _ _ =
    if W.get_state w then (
      L.show (* ~from:Anim.Top *) room;
      L.fade_in room)
    else (
      L.hide (* ~towards:Anim.Top *) room;
      L.fade_out room)
  in
...
let example25 () =
      ...
      [
        ...
        L.superpose
          [
            ...
            L.tower
              [
                L.flat_of_w [ W.label "   Hello there"; W.check_box () ];
                L.empty ~w:0 ~h:100 ();
  ...
  let () =
    ...
    let about () =
      Popup.info "This is Bogue example 25:\n\nA menu bar.." main
    in
    ...
    let file = { label = Text "File"; content = Tower file_menu } in

    (* We define another menu with two submenus: *)
    (* It is ok to re-use an entry or an action because everything is immutable
       here. *)
    let label1 =
      { label = Text "Copy of action 1"; content = Action action1 }
    in
    let label2 =
      { label = Text "Entry with action 2"; content = Action action2 }
    in
    let submenu = [ label1; label2 ] in
    let submenu2 =
      List.concat
        [ submenu; [ { label = Text "submenu"; content = Tower submenu } ] ]
    in
    let submenu3 =
      List.concat
        [
          submenu2;
          [ separator ];
          [ { label = Text "submenu2"; content = Tower submenu2 } ];
        ]
    in
    let menu2 =
      { label = Text "This a long menu title"; content = Tower submenu3 }
    in
...
let example26 () =
  ...
  let action_enter _ w2 _ =
    print_endline "action_enter";
    L.animate_w room (Avar.fromto ~duration:100 (L.width room) 210);
    Label.set (W.get_label w2) "Mouse entered"
  in
  let ce = W.connect box l action_enter [ T.mouse_enter ] in
  let action_leave _ w2 _ =
    print_endline "action_leave";
...
let example26bis () =
  ...
  let enter _ =
    print_endline "action_enter";
    L.animate_w room (Avar.fromto ~duration:100 (L.width room) 210);
    Label.set (W.get_label l) "Mouse entered"
  in
  let leave _ =
    print_endline "action_leave";
    L.animate_w room (Avar.fromto ~duration:100 (L.width room) 200);
...
let example27 () =
  ...
  let long =
    L.tower ~sep:0
      [ debut; td (); td (); td (); td (); td (); td (); td (); fin () ]
  in
  let short =
    L.tower ~sep:0 [ td (); fin () (* ; td (); td (); td (); td () *) ]
  in
...
let example28 () =
  ...
  let board = of_layout layout in
  let _ =
    Timeout.add 5000 (fun () -> print_endline "HELLO!---------------------")
  in
  run board
...
let example29 () =
  let radio =
    Radiolist.vertical ~selected:2
      [| "Only one can be selected"; "AAA"; "BBB"; "CCC" |]
  in
...
let example30 () =
  let radio =
    Radiolist.vertical [| "At most one can be selected"; "AAA"; "BBB"; "CCC" |]
  in
  let label = W.label "   Please make your choice   " in
  let action _ l _ =
    let sel =
      Utils.(
        default (map_option (Radiolist.get_index radio) string_of_int) "nothing")
    in
    Label.set (W.get_label l) (sprintf "You have selected: %s" sel)
  in

  let cs =
    List.map
      (fun w -> W.connect w label action T.(update :: buttons_down))
...
let example31 () =
  Random.self_init ();
  let sizes =
    Array.init (Array.length europe) (fun _ -> 100 + Random.int 100)
  in
  let generate i = L.resident ~h:sizes.(i) (W.label europe.(i)) in
  let long =
    Long_list.create ~w:300 ~h:400 ~generate ~length:(Array.length europe)
      ~max_memory:700000 ()
  in
...
let example32 () =
  ...
  let generate i =
    let background =
      if i mod 2 = 0 then Some (L.color_bg Draw.(transp (pale (pale green))))
      else None
    in
    L.resident ~h:50 ~w ?background (W.label (string_of_int i))
  in
  (* for very long lists a height_fn should be provided, otherwise it can be
     very slow (and blocking) *)
  let height_fn _ = Some 50 in
  let long =
    Long_list.create ~w ~h:400 ~generate ~height_fn ~length:100000
      ~max_memory:700000 ()
  in
...
let example33 () =
  ...
  let generate i =
    let background =
      Some (L.color_bg Draw.(set_alpha (27 * i mod 255) green))
    in
    L.resident ?h:(height_fn i) ~w ?background (W.label (string_of_int i))
  in
...
let desc34 =
  "a very Long List (one million entries) with persistent checks and nonlinear \
   slider"
...
let example34 () =
  ...
  let generate i =
    ...
    W.on_click ~click b;
    (* we need to add connections dynamically *)
    let bl = L.resident ~w:20 ~h b in
    let l = L.resident ~w:(w - 20) ~h (W.label (string_of_int i)) in
    L.flat ~sep:0 ~margins:10 ?background [ bl; l ]
  in
  let height_fn _ = Some (h + 20) in
  let long =
    Long_list.create ~w ~h:400 ~generate ~height_fn ~linear:false ~length
      ~max_memory:900000 ()
  in
    ...
    Table.
      {
        ...
        rows =
          (fun i ->
            L.resident (W.label (string_of_int (String.length europe.(i)))));
        compare =
          Some
            (fun i j ->
              compare (String.length europe.(i)) (String.length europe.(j)));
        width = Some 70;
      }
  in
...
let example37 () =
  ...
  let click b =
    ...
    ignore (Mixer.play_chunk mixer sound)
  in
  W.on_click ~click b;
  let td = W.text_display lorem in
  let box = W.box ~style:round_image_box () in
...
let desc38 =
  "Load SVG at different sizes. The first one is treated as a pattern and the \
   second one as an image."
...
let example38 () =
  ...
  (* One can load an svg image as a background; it will be repeated as a pattern
     to fill the box: *)
  let bg =
    Image.create_from_svg ~width:300 ~height:100
      "%assets/images/w3c-logo-white.svg"
  in
  ...
  (* one can load an svg image as a widget; it will be scaled to fit the
     size: *)
  let img =
    W.image_from_svg ~h:300
      ~bg:Draw.(opaque @@ find_color "darkseagreen")
...
let example39 () =
  ...
  let line0 =
    L.flat
      [
        ...
        L.resident ~background (W.label "Room 3");
      ]
  in
  let line1 =
    L.flat
  ...
  let line2 =
    L.flat [ room3; L.resident ~background (W.label "Bottom right") ]
  in
  Space.make_hfill room3;
  Space.full_width line2;

  let center_area =
    L.tower ~sep:0 [ L.resident ~background (W.label "Vertical fill") ]
  in
  ...
  let layout =
    L.tower ~sep:5
      [
        ...
        L.resident
          (W.label
             "A flat set to full_width and with hfill between rooms 2 and 3:");
        line1;
        center_area;
...
let example39bis () =
  ...
  let topline =
    L.flat ~scale_content:false ~margins:0
      [ L.resident ~background (W.label "Top"); right ]
  in
  let l =
    L.tower ~scale_content:false
      [
        topline;
...
let example40 () =
  let b1 = W.check_box () in
  let l =
    W.text_display ~w:100 ~h:80 "Click on the button to rearrange layout"
    |> L.resident
  in
  let style =
    let open Style in
    create
      ~border:(mk_border ~radius:5 no_line)
      ~background:
        (gradient ~angle:45. Draw.[ opaque sandybrown; opaque cornsilk ])
      ()
  in
  ...
  L.setx layout 0;
  (* this is a way to disable automatic resizing *)
  (* L.set_width layout 400; *)
  let click w =
    if W.get_state w then (
      print_endline "Rearranging layout to a flat layout";
      L.reflat layout)
    else (
      print_endline "Rearranging layout to a tower layout";
      L.retower layout)
  in
  W.on_click ~click b1;
...
let example41 () =
  let image =
    W.image ~w:1024 ~h:768 "%assets/images/nasa_black_hole_cygx1_ill.jpg"
  in
  ...
  let make_btn x y text =
    ...
    let r =
      L.tower ~name:"game button" ~margins:0
        [ L.resident ~w:100 ~h:40 ~background:(L.style_bg style) (*b*) l ]
    in
    L.setx r x;
    L.sety r y;
    r
  ...
  print_endline
    (Printf.sprintf "quit_btn pos = (%i,%i)" (Layout.getx quit_btn)
       (Layout.gety quit_btn));
  let entries =
    let open Menu in
    [
      {
...
let example43 () =
  ...
  let top =
    W.text_display ~w:400 ~h:40
      "The layout on the right is just a 'screenshot' of the layout on the \
       left. It is not active."
  in
  let l = W.label "We snapshot this pack:" in
  let b = W.check_box () in
  let box = W.box ~style:round_image_box () in
  let background = L.theme_bg in
  let room = L.tower ~background [ L.resident l; L.flat_of_w [ b; box ] ] in
  ...
  let layout =
    L.tower ~align:Draw.Center
      [
        L.resident top;
        L.flat ~align:Draw.Center [ L.resident left; L.flat [ room; snap ] ];
      ]
  in
  let board = of_layout layout in
  run board
...
let example44 () =
  ...
  let td =
    W.text_display
      ("Leave the mouse cursor on the buttons to see the tooltips!\n\n" ^ lorem)
  in
  ...
  Popup.tooltip "Tooltip below the button" ~position:Popup.Below ~target:target'
    b' layout;
...
let example45 () =
  ...
  let b = W.box ~style ~w:50 ~h:50 () in
  (* OK *)
  let bg = L.color_bg Draw.(transp green) in
  let l = L.flat ~margins:50 ~shadow ~background:bg [ L.resident b ] in
  (* BUG *)
  let l2 =
    L.flat ~margins:50 ~shadow ~background:(L.style_bg style)
      [ L.empty ~w:50 ~h:50 () ]
  in
...
let example49 () =
  ...
  List.iter
    (W.on_click ~click:(fun _ ->
         if W.get_state b then begin
           Sdl_area.enable element;
           if not (Sdl_area.has_element area element) then (
             print_endline "new circle";
             Sdl_area.add_element area element)
         end
  ...
  let options =
    L.flat ~align:Draw.Center
      [ L.flat_of_w [ b; l ]; Space.hfill (); L.resident cb ]
  in
  Space.full_width options;

  let layout = L.tower [ options; L.resident a ] in
  run (of_layout layout)
...
let run_examples () =
  ...
  let to_run =
    List.tl (Array.to_list Sys.argv) |> List.filter (fun s -> s <> "-h")
  in
  ...
  let exs =
    try List.map (fun key -> (key, List.assoc key examples)) to_run
  ...
  if help then (
    ...
    print_endline
      "Where `id1`, `id2`, etc. are the identifiers of the example demos you \
       whish to run. If no `id` is given, all examples are selected.";
    print_endline
      "Use `-h` if you want to print the description without running the demo.";
    print_newline ());
  Stdlib.exit 0

(* Attention le 16 ne marche pas après le 15: on reste bloqué sur Thread:
   Waiting for locked variable to unlock...==> corrigé *)
