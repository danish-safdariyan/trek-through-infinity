...
(* type t (** The type representing the user's list of tasks *)
   ...
   val remove_task : t -> Task.t -> int -> t (** [remove_task] removes a task by
   its ID from the task list *)

   val edit_task : t -> Task.t -> int -> t (** [edit_task] edits an existing
   task in the task list *)

   val get_task : t -> int -> Task.t (** [get_task] gets a task based on an ID
