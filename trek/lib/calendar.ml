...
module Calendar = struct
  ...
  type t = (string, Event.Event.t list) Map.t
  ...
  let remove_event calendar date event_id =
    let events = try Map.lookup date calendar with Not_found -> [] in
    let filtered_events =
      List.filter (fun e -> Event.Event.get_id e <> event_id) events
    in
  ...
  let edit_event calendar date event_id updated_event =
    ...
    let updated_events =
      List.map
        (fun e -> if Event.Event.get_id e = event_id then updated_event else e)
        events
    in
  ...
  let list_all_events calendar =
    ...
    |> List.map (fun (_, events) -> List.map Event.Event.to_string events)
